...
void MotorController::setupMotors(void) {
  ...
  // Underlying motor initializiation
...
void MotorController::calibrate(void) {
  ...
  bool x_done = digitalRead(A0) == LOW;
  bool y_done = digitalRead(A1) == LOW;
  ...
  while (true) {
    ...
    x_done = digitalRead(A0) == LOW;
    y_done = digitalRead(A1) == LOW;
    if (x_done && y_done) {
      break;
    }
  }
...
/**
 * Stops the actuator from coming out or coming in
 */
void MotorController::stopActuator(void){
  digitalWrite(ACTUATOR_DOWN, LOW);
  digitalWrite(ACTUATOR_UP, LOW);
}
